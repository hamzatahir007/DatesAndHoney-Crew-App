...
const SuggestionScreen = ({ navigation, route }) => {
  ...
  const [image1, setImage1] = useState(data?.image1 ? data?.image1 : null);
  const [image2, setImage2] = useState(data?.image2 ? data?.image2 : null);
  const [image3, setImage3] = useState(data?.image3 ? data?.image3 : null);
  const [image4, setImage4] = useState(data?.image4 ? data?.image4 : null);
  const [image5, setImage5] = useState(data?.image5 ? data?.image5 : null);
  const [image6, setImage6] = useState(null);
  const [about, setabout] = useState(data?.Bio ? data?.Bio : null);
  ...
  const onSaveChanges = async () => {
    if (imageArray.length < 1 || !about || !tips
    ) {
      if (imageArray.length < 1) {
        ToastAndroid.show("please select atleast one Suggested Images", ToastAndroid.SHORT);
      }
      else if (!about) {
